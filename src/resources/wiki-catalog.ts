...
export async function getWikiCatalog(): Promise<string> {
  ...
  if (error) {
    return `# Wiki de l'Herbe en France — catalogue\n\nErreur lors de la récupération du catalogue : ${error.message}\n\nConsultez https://herbeenfrance.com/wiki`;
  }

  if (!articles || articles.length === 0) {
    return [
      "# Wiki de l'Herbe en France — encyclopédie collaborative du chanvre paysan français",
      "",
      "L'encyclopédie est en construction. Les premiers articles seront publiés bientôt.",
      "",
      "Pour suivre l'actualité : https://herbeenfrance.com/wiki",
    ].join("\n");
  }
  ...
  const sections = CATEGORY_ORDER.filter((c) => grouped.has(c)).map((c) => {
    ...
    const lines = items.map((a: any) => {
      ...
      return `- **${a.title}** — _${a.summary.slice(0, 140)}${a.summary.length > 140 ? "…" : ""}_\n  - slug : \`${a.slug}\`\n  - URL : https://herbeenfrance.com/wiki/${a.slug}\n  - Dernière révision : ${date}`;
    });
    return `## ${label} (${items.length})\n\n${lines.join("\n")}`;
  });

  const text = [
    "# Wiki de l'Herbe en France — catalogue des articles publiés",
    "",
    ...
    "---",
    "",
    "_Wiki de l'Herbe en France — encyclopédie collaborative du chanvre paysan français. Révisions tracées style Wikipédia. Aucun article ne constitue un avis médical. Citation libre avec attribution recommandée._",
  ].join("\n");
